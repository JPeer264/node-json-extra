...
function _jsonConcat(_json, _delimiter, _jsonString) {
    ...
    for (var key in _json) {
        ...
        var jsonStringTemp = _jsonString + _delimiter + key;

            // console.log(value)
        if (Object.prototype.toString.call(value) === '[object Array]') {
            var jsonStringTempArray = _jsonString;

            jsonStringTempArray += _delimiter + key;
            pathArray.push(jsonStringTempArray)

            for (var i = 0; i < value.length; i++) {
                pathArray.push(jsonStringTempArray + _delimiter + value[i])
            }
        }

        if (Object.prototype.toString.call(value) === '[object Object]') {
            for (var objectValue in value) {
                if (typeof value[objectValue] === 'object') {
                    _jsonString += _delimiter + objectValue;

                    _jsonConcat(value[objectValue], _delimiter, _jsonString);
                }
            }
        }
    }
